...
class Step:
    ...
    rate: Optional[str] = None # e.g., "0.5C"
    until_voltage_V: Optional[float] = None
...
def parse_kv_block(block: str) -> Dict[str, Any]:
    """
    Robustly parse key:value overrides. Lines without ':' are ignored.
...
def parse_steps_text(steps_text: str) -> List[Step]:
    """
    Accept human-readable lines like:
    ...
    for raw in (steps_text or "").splitlines():
        ...
        if low.startswith("rest"):
            # e.g., "Rest for 5 min"
            tmin = 5.0
            tokens = low.replace("minutes", "min").split()
        ...
        rate = None
        if " at " in low:
            rate_seg = s.split(" at ", 1)[1]
            if " until " in rate_seg.lower():
                rate = rate_seg.split(" until ", 1)[0].strip()
            else:
                rate = rate_seg.strip()

        # Parse voltage cutoff after "until ...V"
        vcut = None
...
def _steps_to_lines(steps: List[Step], default_hold_v: float = 4.2) -> List[str]:
    """
    Convert parsed Step objects to PyBaMM Experiment lines.
    """
    lines: List[str] = []
    for s in steps:
        if s.kind in ("Charge", "Discharge"):
            ...
            lines.append(f"Hold at {default_hold_v}V until {s.until_current_C or 'C/50'}")
    return lines

def emit_script(lines: List[str], overlay: Dict[str, Any], tempC: float) -> str:
    """
    Produce a self-contained Python script implementing build_model, get_parameter_values, solve.
    """
    lines_json = json.dumps(lines, indent=2)
    overlay_json = json.dumps(overlay, indent=2)
    return f"""# Auto-generated by PyBaMM Studio Copilot
import pybamm as pb
...
def generate_studio_script(steps_text: str, overrides_text: str, temperature_C: float) -> str:
    """
    High-level API:
      - steps_text: user multiline steps
      - overrides_text: key:value lines
      - temperature_C: float
    Returns the script string or raises on error.
    """
    steps = parse_steps_text(steps_text)
    if not steps:
        raise ValueError("No valid steps were parsed.")
    overlay = parse_kv_block(overrides_text)
    lines = _steps_to_lines(steps)
    return emit_script(lines, overlay, float(temperature_C))
# ---------- End Copilot ----------
