...
def _emit_pybamm_script(model_name: str, lines: list[str], overlay: dict, temp_C: float) -> str:
    # ensure stable key names
    overlay = dict(overlay or {})
    script = f'''# Auto-generated by Copilot Chat
import pybamm as pb

def build_model():
    return pb.lithium_ion.{model_name}()
...
def get_parameter_values():
    ...
    try:
        ...
        pass
    # Common capacity key guard
    if "Nominal cell capacity [A.h]" not in params:
        try:
            params.update({{"Nominal cell capacity [A.h]": {overlay.get("Nominal cell capacity [A.h]", 2.0)}})
        except Exception:
            pass
    return params

def solve(model, params, experiment=None):
    # Prefer the provided experiment if Studio passes one in
